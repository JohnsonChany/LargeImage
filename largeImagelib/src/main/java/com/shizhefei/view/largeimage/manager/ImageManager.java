...
import com.shizhefei.view.largeimage.model.CacheData;
import com.shizhefei.view.largeimage.model.DrawData;
import com.shizhefei.view.largeimage.model.LoadData;
import com.shizhefei.view.largeimage.model.Position;
...
public class ImageManager {
    ...
    private final int BASE_BLOCK_SIZE;
    ...
    public ImageManager(Context context) {
        ...
        BASE_BLOCK_SIZE = width;
        Log.d("cccc", "BASE_BLOCK_SIZE: " + BASE_BLOCK_SIZE);
    }

    public void start(OnImageLoadListener invalidateListener) {
        this.onImageLoadListener = invalidateListener;
    ...
    private Rect madeRect(Bitmap bitmap, int row, int col, int scaleKey, float imageScale) {
        int size = scaleKey * BASE_BLOCK_SIZE;
        Rect rect = new Rect();
        rect.left = (col * size);
        rect.top = (row * size);
        ...
        if (!needShowPositions.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (scaleKey / scale == 2) {// 这里都是大于scale的.,缩小的图，单位区域大
                    ...
                    // 单位图片的真实区域
                    int size = scale * BASE_BLOCK_SIZE;
    ...
    private class LoadHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == MESSAGE_LOAD) {
                if (loadData.mFactory != null) {
                    try {
                        ...
                        if (onImageLoadListener != null) {
                            onImageLoadListener.onImageLoadFinished(loadData.mImageWidth, loadData.mImageHeight);
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                ...
                if (imageData == null) {
                    int imageBlockSize = BASE_BLOCK_SIZE * data.scale;
    ...
    public interface OnImageLoadListener {
        ...
        void onImageLoadFinished(int imageWidth, int imageHeight);
    }
